...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['Title']) or  $_POST['Descriptions']) {
        ...
        if (empty($Title) || empty($Descriptions))  {
          ...
          $sql = "INSERT INTO USERS_NOTEBOOK_DATA (`user_id`,`Title`,`Descriptions`) 
          VALUES ('$userid1', '$Title' ,'$Descriptions')";
  ...
  //  collecting  if  user request i is to update the data .
    ...
    // if this true the user want to delete the spefic id 
      ...
      if (mysqli_query($conn, $sql1)) {
        ...
        // this displays if the spefied data by user is sucessfully updated 
        echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>
...
<div class="container" my-3>
  <table class="table" id="myTable">
    ...
    <tbody>
    <?php 
      //$sql = "SELECT * FROM USERS_NOTEBOOK_DATA WHERE `user_id`= '$userid1'";

      $sql = "SELECT USERS_NOTEBOOK_DATA.id, USERS_NOTEBOOK_DATA.Title, USERS_NOTEBOOK_DATA.Descriptions FROM USERS_NOTEBOOK_DATA INNER JOIN users ON USERS_NOTEBOOK_DATA.user_id = users.id WHERE users.email ='".$email."'";
